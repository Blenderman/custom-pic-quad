...
static inline HMC_RC    _HMC_ConvertStream(    byte* Data,
    ...
    // coordinate system ("NED" coordinates) with Z-axis
    // pointing down and considering the orientation of
    // the sensor on the board, we make the following
    // adjustments:
    //    Xres = -X;
    //    Zres = -Z;
    //    Yres =  Y;
...
static inline void    _HMC_NormHMCData(Vector* MData)
    {
    //-----------------------------------------------
    // Adjust sample for sensitivity
    //-----------------------------------------------
    VectorScale(MData, _HMC_Gain, MData);
    //-----------------------------------------------
