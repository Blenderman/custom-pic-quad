...
uint	DCMPerformStep(	ulong		TS,			// Timestamp of the cycle
	...
	if (_DCMReady)
		{
		VectorScale(&AccError, _DCMAccKp, &AccPterm);	// Normal convergence (weight)
		VectorScale(&AccError, (_DCMAccKi * TimeStep), &AccIterm);
		}
	else
		{
		VectorScale(&AccError,    0.1,    &AccPterm);	// Fast convergence 
	...
	VectorScale(	pGyroRaw, TimeStep, &RotationDelta);
	//--------------------------------------------------------------------------
