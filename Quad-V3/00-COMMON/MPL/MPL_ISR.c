...
void __attribute__((interrupt, no_auto_psv)) MPL_Interrupt(void)
    ...
    // (interrupt may be "forced" by asynchronous read routines
...
void    _MPLCallBack(uint            ClientParam,
    ...
    switch (_MPL_State)
        ...
        case    1:        // Interrupt after device address
            // Slave send ACK (confirmed in I2C ISR of the I2C Library);
        ...
        case    18:        // Interrupt after CtrlR1 write
            ...
            if (_MPL_PortLvl > 0) // Data read was in responce to DRDY
                ...
                //-------------------------------------------------
                // At this point _MPL_Ready > 0 -> Sample is ready!
                //-------------------------------------------------
                }
            //-----------------------------------------------------------
            // We need to update the "last read timestamp" so that Async
            // read routines may trigger interrupt if maximum time for
            // sample acquisition has expired.
            //-----------------------------------------------------------
