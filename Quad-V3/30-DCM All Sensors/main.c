...
int main(void)
    ...
    if ( MPU_OK != MPUInit(0, 3, MPU_GYRO_2000ds, MPU_ACC_2g) )
        BLIDeadStop("EG", 2);
    //-------------------------------------------------------------------
    // </editor-fold>
    //------------------------------------------------------------------- 
    ...
    // Initialize DCM
    //------------------------------------------------------------------
    if ( HMC_OK != HMC_AsyncReadWhenReady(&SensorData.HMCSample) )
        BLIDeadStop("SHMC", 4);
    //-----------------------------------------------------
    if ( MPU_OK != MPUAsyncReadCombined(&SensorData.MPUSample) ) 
        BLIDeadStop("SMPU", 4);
    //-----------------------------------------------------
    DCMSetOptions(TRUE, FALSE, FALSE);
    //-----------------------------------------------------
    DCMReset(&SensorData.MPUSample.A, &SensorData.HMCSample.M);
    //==================================================================
    // Main Loop
    //------------------------------------------------------------------
    BLISignalOFF();
    ...
    while (TRUE)
        ...
        if ( MPU_OK != MPUAsyncReadCombined(&SensorData.MPUSample) )
            BLIDeadStop("SOS", 3);
        //-----------------------------------------------------
        HMC_AsyncReadIfReady(&SensorData.HMCSample);
        //-----------------------------------------------------
        MPLAsyncReadIfReady(&SensorData.MPLSample);
        //-----------------------------------------------------
        ADCGetBatteryStatus(&SensorData.BatStatus);
        //-----------------------------------------------------
        if (DCM_OK != DCMUpdate( SensorData.MPUSample.TS,	// Timestamp of the cycle
                                &SensorData.MPUSample.G,
                                &SensorData.MPUSample.A,
                                &SensorData.HMCSample.M,
                                &SensorData.DCMOut))
            BLIDeadStop("SSS", 3);
        Step++;
        //-----------------------------------------------------
        if (Step >= 100)
            ...
            BLISignalFlip();
            //-------------------------
            SDLPostIfReady( (byte*)&SensorData.DCMOut, 
                            sizeof(SensorData.DCMOut));
            }
        //-------------------------
        TMRWaitAlarm(Alarm);
        }
